...
public class View extends Application {
    ...
    private static Repository createRepository(IStmt statement) {
        ...
        MyIRandIntKeyDict<ITuple<Integer, MyIList<Integer>>> barrierTable = new MyRandIntKeyDict<>(generator);
        PrgState state = new PrgState(myStack1, myDictionary1, myList1, fileTable1, heap, lookTable, barrierTable
                , statement, 1);
...
//                new CompStmt(new NewStmt("a", new ConstExp(22)),
//                        new CompStmt(new ForkStmt(forkStmt,gen),
//                                new CompStmt(new PrintStmt(new VarExp("v")),
//                                        new PrintStmt(new ReadHeapExp("a"))))));
//
...
//                                                        continuareaEx18)))))))));
        ...
        // (default print(300));
        //print(300)
        IStmt ex19 = new CompStmt(new AssignStmt("a", new ConstExp(1)),
        ...
        ///checking if prgstate id are unique
        ///v=1;fork(v=2);fork(v=3)
        IStmt ex20 = new CompStmt(
                new AssignStmt("v", new ConstExp(1)),
                new CompStmt(new ForkStmt(new AssignStmt("v", new ConstExp(2)),gen),
                        new ForkStmt(new AssignStmt("v", new ConstExp(3)),gen))
        );
